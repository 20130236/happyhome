...
public class OrderService {
    ...
    public Order getOderById(int id) {
        ...
        String sql;
        try {
            sql = "SELECT order_id, user_name, payment, total_money, fee, date_order, transport, status FROM orderr WHERE order_id = " + id;
            pst = DBConnection.getConnection().prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs.next()) {
                order = new Order(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getLong(4), rs.getInt(5), rs.getDate(6), rs.getString(7), rs.getInt(8));
    ...
    public void addOder(Order o) {
        String sql = "INSERT INTO orderr (order_id, user_name, payment, total_money, fee, date_order, transport, status) VALUES(?,?,?,?,?,?,?,?)";
    ...
    public int getMaxMHD() {
        ...
        int result = 0;
        PreparedStatement ps;
        String sql = "SELECT MAX(order_id) FROM orderr";
        try {
            ps = DBConnection.getConnection().prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
